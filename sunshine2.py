...
# os.environ["PROJ_LIB"] = 'C:\\Users\\Khadija\\Anaconda3\\Library\\share'
# /Users/tanmaybaweja/Desktop/data
...
def compute_average_temp(temperatures):
    ...
    return float(sum)/float(len(temperatures))
...
def compute_average_change(temperatures):
    ...
    return float(sum)/float(len(change))
...
def compute_average_changes_dict(station_info_dict,
    ...
    average_changes_dict_ = {}

    station_ids = list(temperature_data_dict.keys())

    for j in station_ids:
        avgmeantemp = []
        for i in range(end_year - start_year):
            temps = j[start_year]
            tempvalue = compute_average_temp(temps)
            avgmeantemp.append(float("%.2f"%tempvalue))
            start_year+=1
    
        avgchange = compute_average_change(avgmeantemp)
        avgchange = float("%.4f"%avgchange)
        temp_dict = {j:avgchange}
        average_changes_dict_.update(temp_dict)

    return average_changes_dict_
...
def compute_top_average_change_dict(average_changes_dict,n):
    ...
    reduced_dict = {}
    keys = list(average_changes_dict.keys())
    ...
    values = sorted(list(average_changes_dict.values()))
    new_values = values[:n]

    for i in range(len(keys)):
        key = get_key(new_values[i])
        temp_dict = {key:new_values[i]}
        reduced_dict.update(temp_dict)
...
def make_average_change_dict_for_map(station_info_dict, average_change_dict):
    ...
    keys = list(average_change_dict.keys())
    for i in keys:
        station_values = station_info_dict.get(key)
        avgchange_value = average_change_dict.get(key)
        temp_dict = {station_values['station_name']:(station_values['lat'], station_values['lon'], avgchange_value)}
        average_change_dict_for_map.update(temp_dict)
...
def draw_top_average_changes_map(top_average_changes_dict_for_map,
    ...
    station_names = list(top_average_changes_dict_for_map.keys())
    station_data = list(top_average_changes_dict_for_map.values())
    print('Top' + num_top_values + 'Average Annual Temperature Changes Between' + start_year + 'and' + end_year + ':')

    for i in range(len(station_names)):
        print(station_names[i] + '(' + station_data[i][0] + ',' + station_data[i][1] + ') :' + station_data[i][2] + 'degrees')
